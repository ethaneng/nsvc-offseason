...
} from "./RegistrationCardButtons";
import { getParticipantsForRegistration } from "@/lib/serverActions";

type Props = {
  registrationType: Tables<"Registration_Type">;
  event_id: number;
};
...
async function RegistrationCard({ registrationType: rego, event_id }: Props) {
  ...
  if (sessionError) {
    ...
    return (
      <p>
        Something went wrong fetching data for the current user. Please try
        again later.
      </p>
    );
  }
  ...
  if (session) {
    ...
    if (error) {
      ...
      return (
        <p>
          Something went wrong fetching data for this registration type. Please
          try again later.
        </p>
      );
    }
    ...
    <Card className={hasRegisteredThisType ? "border-primary" : ""}>
      ...
      <CardFooter>
        ...
        {session && hasRegisteredThisType && (
          <UnregisterCardButton rego_id={rego.id} user_id={session.user.id} />
        )}
        {session && !hasRegisteredAnotherType && !hasRegisteredThisType && (
          <RegisterCardButton rego_id={rego.id} user_id={session.user.id} />
        )}
